...
class LocationSensitiveAttention(BahdanauAttention):
    ...
    def __init__(self, query_dim, attn_dim, filters=32, kernel_size=31, score_mask_value=-float("inf")):
        ...
        self.conv = nn.Conv1d(2, filters, padding=(kernel_size - 1) // 2, kernel_size=kernel_size, bias=False,stride=1,dilation=1)
    ...
    def init_attention(self, processed_memory):
        ...
        self.attention_weights = processed_memory.data.new(b, t).zero_()
    ...
    def get_energies(self, query, processed_memory):
        ...
        # Location feature
        location = torch.cat((self.attention_weights.unsqueeze(1),self.cumulative.unsqueeze(1)),dim=1)
    ...
    def get_probabilities(self, energies):
        # Current attention
        self.attention_weights = nn.Softmax(dim=1)(energies)

        # Cumulative attention
        self.cumulative = self.cumulative + self.attention_weights
